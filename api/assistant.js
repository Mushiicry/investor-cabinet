          ...
          content: [
            {
              ...
              text: [
                ...
                "Если context.answerFocus.type = detailed_health_components, сначала используй видимые компоненты здоровья и их score. Формулы — только как переданная расшифровка, без новой математики.",
                "Если context.answerFocus.type = visible_overview_recommendations, отвечай только по видимым карточкам рекомендаций справа.",
                "Если context.answerFocus.type = visible_portfolio_staking, отвечай только по visibleInvestmentPositions[].staking.",
                "Если context.answerFocus.type = health_page_overview, объясняй вкладку Здоровье как страницу: назначение, основные блоки и что проверять. Не делай подробный расчет Health Factor.",
                "Если context.answerFocus.type = portfolio_daily_report, используй context.dailyReport. Структура: итог; вложено/текущая стоимость/общий P&L; P&L 24H при наличии; инвестиционные позиции; кэш/резерв отдельно; Health Factor; Fear & Greed при наличии; главный риск.",
                "Для portfolio_daily_report не называй общий P&L дневным. Дневной результат брать только из context.dailyReport.dailyPnl.",
                "Для portfolio_daily_report позиции бери из context.dailyReport.investmentPositions, а кэш/резерв из context.dailyReport.cashAndReserveRows.",
                "Для portfolio_daily_report индекс страха и жадности называй только если context.dailyReport.dataAvailability.hasFearGreed=true.",
                "",

...
## 2026-07-22 — Патч 3, этапы 2-5А: проверка добора и калькулятор сделки

Назначение:
превратить экран «Проверка» в предварительный шлюз решения: до покупки система
показывает, разрешён ли актив, сколько можно купить, что будет со средней входа,
выживаемостью и здоровьем портфеля.

Что изменено:
- добавлен чистый движок решений со статусами: разрешено, разрешено с лимитом,
  осторожно, блокировка и ждать;
- проверки разделены на жёсткие причины блокировки и предупреждения;
- добавлены карманы торгового капитала: резерв, фьючерсы, усреднение, спот,
  металлы и акции;
- ручная крипта, усреднение, металлы, акции и фьючерсы больше не смешиваются в
  один свободный остаток;
- добавлена цена покупки и калькулятор усреднения средней входа;
- добавлена проверка выживаемости после сделки;
- добавлена автоматическая проверка качества криптоактива: CoinMarketCap Top-100
  и Binance Monitoring;
- источник качества активов вынесен из интерфейса: данные идут через программный
  интерфейс, таблицу «Качество активов» и Vercel-прокси Binance;
- токены вне топ-100 и токены Binance Monitoring блокируются автоматически;
- для запрещённых активов нижнее действие стало красной неактивной кнопкой,
  чтобы интерфейс не создавал ощущение разрешённой покупки;
- добавлен прогноз здоровья до/после сделки;
- для запрещённых активов прогноз здоровья после сделки помечается как
  неприменимый: актив запрещён политикой риска;
- добавлены тесты на старый кэш без `assetQuality`, запретные токены, источник
  Binance Monitoring, калькулятор усреднения и здоровье до/после.

Проверено:
- `JASMY`, `NOM` и `GTC` блокируются по качеству актива;
- `SOL` блокируется или ограничивается по лимитам концентрации;
- обычная проверка показывает здоровье до/после;
- запрещённый актив показывает «здоровье после сделки не применяется»;
- проверка выполнялась на живом `localhost` в пользовательском Chrome.

Что не изменено:
- структура таблиц портфеля;
- контракт основного программного интерфейса `/api/investor`;
- Apps Script-учёт покупок и продаж;
- источник фактов по портфелю;
- геометрия основного гексагона здоровья.

---

## 2026-07-22 — Патч 3, этап 6А: журнал решений v1

Назначение:
начать журналировать не только факт сделки, а качество решения до сделки:
контекст проверки, статус риска, здоровье до/после, состояние пользователя и
сетап.

Что изменено:
- на странице «Проверка» добавлен блок «Журнал решения»;
- пользователь может указать сетап, состояние и заметку к решению;
- кнопка «Сохранить решение» сохраняет снимок текущей проверки;
- снимок включает актив, класс, сумму, цену, статус решения, причины,
  предупреждения, здоровье до/после, изменение здоровья, выживаемость и новую
  среднюю входа;
- в «Отчётах» добавлена отдельная секция «Журнал решений»;
- журнал решений не смешивается с «Историей сделок» и не считается фактом
  портфеля;
- добавлено удаление ошибочно сохранённой локальной записи;
- журнал v1 хранится локально на устройстве и не меняет Google Sheets;
- добавлены контрактные тесты чтения, записи, удаления и защиты от битого
  локального хранилища.

Проверено:
- сохранение решения из «Проверки»;
- отображение решения в «Отчётах»;
- удаление записи из журнала;
- тестовая запись после проверки удалена;
- проверка выполнялась на живом `localhost` в пользовательском Chrome.

Что не изменено:
- структура Google Sheets;
- Apps Script;
- основной программный интерфейс `/api/investor`;
- бухгалтерская история сделок;
- расчёт портфеля и средней входа.

---

## 2026-07-22 — Патч 3, этап 7А: поведенческий движок v1

Назначение:
подключить журнал решений к дисциплине, чтобы он не был декоративной историей,
а реально защищал проверку сделки и луч «Дисциплина».

Что изменено:
- добавлен поведенческий движок на основе локального журнала решений;
- одна сохранённая блокировка считается предупреждением, а не нарушением:
  фиксация запрета — это дисциплина;
- повторный страх упустить рост, повторные попытки пройти блокировку, решение
  после убытка в рискованном контексте и три решения за сутки включают
  дисциплинарную паузу;
- «Проверка» получает дисциплинарные предупреждения и блокеры;
- при дисциплинарной паузе сделка получает статус «БЛОКИРОВКА»;
- кнопка действия при дисциплинарной паузе стала неактивной и красной:
  «Заблокировано — активна дисциплинарная пауза»;
- «Отчёты» получили карточку «Поведение» со статусом, баллом, счётчиками и
  причинами паузы;
- Health пересчитывает луч «Дисциплина» с учётом локального журнала, частоты
  решений, страха упустить рост и активной паузы;
- тесты закрепляют пустой журнал, одиночную блокировку, повторный страх
  упустить рост и переторговку.

Что не изменено:
- структура Google Sheets;
- Apps Script API;
- финансовые расчёты портфеля;
- источник фактов по позициям, ценам и суммам;
- геометрия основных экранов.
...
## 2026-07-22 — Патч 3, этап 7Б: симулятор шести лучей

Назначение:
довести симулятор здоровья до новой архитектуры гексагона: один рычаг — один
луч здоровья. Симулятор должен показывать путь к улучшению портфеля через
реальные входы движка здоровья, а не через декоративные баллы.

Что изменено:
- старые технические рычаги заменены на шесть смысловых рычагов:
  «Резерв», «Диверсификация», «Концентрация», «Контроль риска»,
  «Выживаемость», «Дисциплина»;
- логика симулятора вынесена из компонента в отдельный модуль;
- рычаг «Концентрация» теперь двигает реальные поля per-asset концентрации:
  балл концентрации, использование лимита, худшую долю, список активов сверх
  лимита и занятые места;
- рычаг «Контроль риска» снижает маржу активной торговли, плечо, число
  активных позиций и риск близкой ликвидации;
- рычаг «Выживаемость» подключает план лимитных ордеров только в пределах
  доступной покупательной способности;
- рычаг «Дисциплина» двигает журнал решений, нарушения, страх упустить рост,
  сделки-месть, переторговку и дисциплинарную паузу;
- добавлены контрактные тесты симулятора здоровья.
...
- контракт программного интерфейса;
- источник фактов по портфелю;
- общая геометрия интерфейса.

---

## 2026-07-22 — Патч 3, внеплановый фикс: рекомендации здоровья

Назначение:
сделать рекомендации на главном экране реальными действиями по улучшению
здоровья портфеля, а не списком уже выполненных состояний.

Что изменено:
- главный экран больше не показывает выполненные рекомендации вроде
  «Контроль риска в норме»;
- выполненные состояния остаются отдельной сущностью достижений и могут
  использоваться в отчётах;
- список рекомендаций ограничен пятью актуальными действиями;
- каждая рекомендация получает прирост здоровья из реальной модели:
  симуляция исправления луча + вес луча в Health Factor;
- рекомендации с нулевым приростом не попадают на главный экран;
- добавлена дедупликация: максимум одна рекомендация на один луч здоровья;
- страница «Здоровье» и главный экран используют единый генератор рекомендаций;
- добавлены контрактные тесты на максимум пять рекомендаций и удаление
  выполненных пунктов.
...
- контракт программного интерфейса;
- источник фактов по портфелю;
- визуальная геометрия карточки рекомендаций.

---

## 2026-07-22 — Патч 3, этап 8: рыночная психология v1

Назначение:
связать живой индекс страха и жадности с проверкой сделки и дисциплиной, не
превращая рынок в отдельный балл здоровья и не подменяя им решение инвестора.

Что изменено:
- рыночная психология получила режим риска: «покупать по плану», «держать
  план», «снижать риск», «защита капитала»;
- в зоне жадности новая покупка риск-актива переводится в осторожный режим и
  требует ручной проверки;
- в зоне эйфории увеличение риска блокируется до выхода рынка из перегрева;
- страница «Проверка» показывает блокировку рынка как отдельную причину и не
  предлагает кнопку уменьшения суммы как будто сделка допустима;
- поведенческий движок учитывает рыночный контекст: страх упустить рост в
  эйфории сразу включает дисциплинарную паузу;
- страница «Сигналы» показывает, что текущая рыночная психология означает для
  проверки сделки;
- добавлены контрактные тесты рыночной психологии, блокировки сделки и
  усиления дисциплины при страхе упустить рост в перегретом рынке.
...
## 2026-07-24 — Патч 3, этап 9А: качество и приоритет ценовых сигналов

Назначение:
начать этап живых данных и уведомлений с безопасного слоя качества сигналов:
ценовая точка должна помогать принять решение, но не выглядеть как разрешение
на сделку.

Что изменено:
- добавлена оценка каждой точки интереса: свежесть проверки, расстояние до
  уровня, приоритет и необходимость пройти проверку риска;
- сработавшая точка теперь формулируется как повод открыть проверку риска, а
  не как сигнал к покупке;
- устаревшие или снятые с дежурства точки поднимаются в предупреждения;
- уведомления по ценовым точкам сортируются по приоритету и ограничивают шум;
- страница «Сигналы» показывает дисциплинарный смысл каждой точки;
- добавлены контрактные тесты на свежесть, приоритет и отправку в проверку
  риска.
...
- контракт программного интерфейса;
- расчёты портфеля;
- источник истины по ценам и сделкам.

---

## 2026-07-24 — Патч 3, этап 9Б: лимиты и повторы ценовых уведомлений

Назначение:
закрыть риск переторговки от живых уведомлений: ценовой сигнал может напомнить
о плане, но не должен превращаться в поток команд к действию.

Что изменено:
- добавлена политика уведомлений по сигналам: дневной лимит, повтор и
  дисциплинарная пауза;
- дневной лимит по умолчанию — 3 напоминания в день;
- повтор по уже сработавшей точке разрешён не чаще чем раз в 6 часов;
- фронт показывает причину ограничения рядом с ценовым сигналом;
- при дисциплинарной паузе сигнал не получает действие «Открыть проверку
  риска», чтобы не подталкивать к импульсивному решению;
- Telegram-скрипт получил тот же дневной лимит, защиту от частых повторов и
  ручной флаг дисциплинарной паузы в Script Properties;
- добавлены контрактные тесты на дневной лимит, повтор, MSK-время и
  дисциплинарную паузу.
...
- контракт программного интерфейса;
- источник фактов по портфелю;
- формулы расчёта портфеля;
- формат листа «Сигналы».

---

## 2026-07-24 — Патч 3: аудит схемы фактора здоровья v2

Назначение:
выполнить следующий roadmap-пункт без кода: проверить текущую Health/Risk
реализацию, подготовить схему фактора здоровья v2 и зафиксировать точные поля,
которые уже есть и которых не хватает.

Что зафиксировано:
- текущая модель уже соответствует risk-first логике и считает 6 независимых
  лучей: резерв, диверсификация, концентрация, контроль риска, выживаемость,
  дисциплина;
- ключевой технический долг — legacy-ключи компонентов: `crypto` фактически
  означает «Выживаемость», `futures` — «Контроль риска», `flexibility` —
  «Дисциплина»;
- план лимитных ордеров поддержан формулой, но ещё не подключён как живой
  источник данных;
- журнал дисциплины сейчас локальный, а не общий источник фактов;
- GOLD контролируется по плечу, но его маржа ещё не включена в лимит 10%
  активной торговли;
- предложен следующий маленький code patch: слой совместимости ключей v2 без
  изменения формул, API и UI.
...
- контракт программного интерфейса;
- UI;
- формулы Health Factor.

---

## 2026-07-24 — Патч 3, этап 9В: лимитные ордера как часть выживаемости

Назначение:
переименовать «Зону интереса» в её настоящий смысл — «Лимитные ордера» — и
подключить сумму активных ордеров к Health Factor.

Что изменено:
- блок на странице «Сигналы» переименован в «Лимитные ордера»;
- в заголовке блока показывается общий депозит по активным лимитным ордерам и
  количество ордеров;
- сумма считается только по активным покупательным строкам: сработавшие,
  сломанные, строки на проверке, продажи и сокращения не входят в депозит;
- live Health Input теперь получает `plannedLimitOrdersUsd` из списка сигналов;
- луч «Выживаемость» перестаёт жить на заглушке и видит фактический план
  лимитных ордеров;
- луч «Дисциплина» получил отдельный небольшой компонент за подготовленный
  план лимитных ордеров;
- добавлены контрактные тесты на расчёт суммы ордеров и влияние плана на
  дисциплину.
...
- Apps Script;
- контракт программного интерфейса;
- формулы портфельного учёта;
- правила исполнения сделок: лимитный ордер всё равно не является разрешением
  на покупку без проверки риска.

---

## 2026-07-24 — Патч 3: слой совместимости ключей Health Factor v2

Назначение:
закрыть технический долг из аудита схемы Health Factor v2 без полной миграции
UI/API: старые ключи остаются рабочими, но у каждого луча появляется
канонический v2-ключ.

Что изменено:
- добавлен `v2Key` для каждого компонента здоровья;
- добавлена карта совместимости:
  `crypto → survival`, `futures → riskControl`, `flexibility → discipline`;
- добавлен обратный поиск:
  `survival → crypto`, `riskControl → futures`, `discipline → flexibility`;
- добавлены функции `toHealthComponentV2Key`, `toLegacyHealthComponentKey` и
  `findHealthComponentByKey`;
- лестница уровней и расчёт прироста рекомендаций начали использовать
  совместимый поиск, поэтому работают и со старыми, и с новыми ключами;
- добавлены контрактные тесты на наличие v2-ключей и поиск по обоим форматам.

Что не изменено:
- формулы Health Factor;
- веса лучей;
- порядок лучей в гексагоне;
- UI;
- структура API;
- Google Sheets;
- Apps Script.
...
## 2026-07-24 — P2: code-split клиентского бандла

Назначение:
закрыть повторяющееся предупреждение сборки о JS chunk больше 500 kB без
изменения интерфейса, данных и логики продукта.

Что изменено:
- добавлен безопасный `manualChunks` в Vite;
- React вынесен в отдельный `react-vendor` chunk;
- Supabase вынесен в отдельный `supabase` chunk;
- главный `index` JS chunk уменьшился примерно с 784 kB до 390 kB;
- предупреждение Vite о chunk больше 500 kB исчезло.

Что не изменено:
- UI;
- маршруты и состояние приложения;
- API;
- Google Sheets;
- Apps Script;
- Vercel config;

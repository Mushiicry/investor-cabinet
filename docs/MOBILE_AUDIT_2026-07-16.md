...
## Резолюция (Claude, 2026-07-16, живая проверка на 375px)

Проверено в браузере постранично. Итог по топ-10:

| # | Кандидат | Вердикт | Действие |
|---|---|---|---|
| 1–3 | Таблицы Отчётов (9–10 fixed-колонок) | **ПОДТВЕРЖДЕНО**: таблицы 1140px клиповались, колонки отрезаны; `.v2-rep-left` без `min-width:0` распирал панель до 1204px | Горизонтальный свайп внутри карточки + `min-width:0`; попутно вся страница Отчётов адаптирована (KPI 2×3, правая колонка на всю ширину, паддинг под навбар) |
| 4–5 | Стейк-тогл/карта | Оверрайды работают (чинились ранее в этот же день) | — |
| 6–8 | Health core/breakdown/top | Оверрайды работают | — |
| 9 | `.v2-re-body` 380px | Не воспроизводится: страница Риск 375px без переполнений | — |
| 10 | realized-label | Не воспроизводится: плашка переносится корректно | — |
| + | `.v2-sig-rule-name` (из таблицы находок) | **ПОДТВЕРЖДЕНО**: имена правил обрезались (scrollWidth 137 vs 109) | Перенос на 2 строки ≤480px |
| + | `.v2-alert-card`, `.v2-psy-stance`, `.v2-hc-chip-name` | Не воспроизводятся: клипов нет | — |

Метод сверки: `document.documentElement.scrollWidth` == 375 на всех страницах,
поэлементный обход на выход за вьюпорт, `scrollWidth > clientWidth` для nowrap-узлов.
Элементы мобильной шторки сайдбара за левым краем — норма (выдвижная панель).

Вывод: статический аудит дал 2 подтверждённых дефекта из 10+ кандидатов —
оба починены в этом же коммите. Остальные кандидаты уже были накрыты
мобильными оверрайдами (аудит честно это пометил).

...
- 🟡 **Сигналы** — базовый экран собран: психология рынка, стратегия F&G,
  health-факторы и **живая «Зона интереса»** (лист «Сигналы» → API → сайт,
  минутный триггер Apps Script → Telegram-алерт при касании цены).
  Осталось: расстояние до триггера в UI, группировка по активу,
  reserve warnings и macro signals;
- 🔲 **Настройки** — policy limits editor, risk thresholds.

**Price alerts (2026-07-21, live):**

Лист «Сигналы» — источник истины по зонам интереса. Apps Script
...
- 🔲 расстояние до триггера (% и $) прямо в строке — главный недостающий
  сигнал для решения;
- 🔲 сортировка по близости срабатывания вместо порядка строк листа;

...
## 2026-07-16 — Полная ревизия таблицы + политика лимитов + мобильная волна 2

- Ревизия источника истины (26 листов, слепок с формулами): 0 ошибок после фиксов.
  Волна 1: Риск B37-B40 и описания X-блока выровнены после вставки SPCXB,
  диапазоны 2:16 -> 2:100. Волна 2: лист «Фьючерсы» синхронизирован с «Расчетами»
  (BTC SHORT закрыты, MNT LONG добавлен, GOLD перевешен формулами), листы
  «Портфель» и «HL_DEBUG» удалены, «Отчет» перевешен на Обзор/Риск.
- Health: хардкоды 84 заменены 6-компонентной моделью сайта формулами;
  сайт 85 = таблица 85 (одна модель).
- F&G-обновлятор починен: Настройки обновляются живым alternative.me
  (были застывшие с 18.04).
- Политика лимитов владельца: крипта 60% / акции 10% / металлы 10% /
  фьючерсы 10% / резерв 30% с полом 10% (riskRules.ts + Риск B21).
- API: overview.realizedPnl / realizedPnlPct (реализованный профит);
  на странице Портфель плашка «Реализовано за всё время».
- Умная рекомендация диверсификации: конкретные $ и классы из meta модели.
- Мобильная волна 2: симулятор (плотный фон), breakdown, DCA-зоны,
  BTC-график full-bleed + фулскрин, стейблы, стейк-карта, разделители позиций.
- Сверка цифр (SHEET_CROSSCHECK_2026-07-16.md): все FAIL ложные, ядро чистое.
- Коммиты: d56f0f7, a5faabc, 9da79b9, 57fa128, 7a4d743. Деплой /exec @43.

...
// Синхронизация формулы Резерва (пол 10%) с моделью сайта — политика 2026-07-16.
// Идемпотентно, можно запускать повторно.
function syncReserveFloorFormula20260716() {
  var risk = SpreadsheetApp.getActiveSpreadsheet().getSheetByName('Риск');
  if (!risk) throw new Error('Нет листа Риск');
  risk.getRange('B21').setFormula(
    '=ROUND(IF(B4<=0;0;IF(B4<0,1;B4/0,1*30;IF(B4<0,3;30+(B4-0,1)/0,2*70;IF(B4<=0,6;100;MAX(0;(1-B4)/(1-0,6)*100)))));0)'
  );
  risk.getRange('D21').setValue('Коридор 30–60% = 100. Пол политики 10% (=30 баллов) — допустим при полном рынке.');
  return 'OK';
}
...
function applySheetRevisionWave2_20260716() {
  ...
  if (risk) {
    var comps = [
      ...
      [21, 'Резерв (20%)',
       '=ROUND(IF(B4<=0;0;IF(B4<0,1;B4/0,1*30;IF(B4<0,3;30+(B4-0,1)/0,2*70;IF(B4<=0,6;100;MAX(0;(1-B4)/(1-0,6)*100)))));0)',
       'Коридор 30–60% = 100. Пол политики 10% (=30 баллов) — допустим при полном рынке.'],

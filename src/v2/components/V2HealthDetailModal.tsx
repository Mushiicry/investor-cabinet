...
const WHAT: Record<HealthComponentKey, string> = {
  ...
  futures:
    "Дисциплина фьючерсов: оценивается по двум правилам — вес фьючерсов ≤10% портфеля И плечо ≤2x на альтах, ≤3x на BTC и золоте. Балл = худшее из двух. Высокое плечо экспоненциально повышает риск ликвидации при движении против позиции.",
  concentration:
...
const HOW: Record<HealthComponentKey, string[]> = {
  ...
  futures: [
    ...
    "Сократите вес всех фьючерсных позиций до ≤10% портфеля",
...
function whyText(c: HealthComponent, portfolio: V2Portfolio): string {
  ...
  if (key === "futures") {
    ...
    // 3) Вес близок к критическому лимиту 10%
    if ((m?.weightScore ?? 100) < 60 && weightPct != null) {
      return `Вес фьючерсов ${weightPct}% приближается к критическому лимиту 10%. Чем ближе к лимиту — тем ниже балл. Сократите экспозицию.`;
    }
    if (weightPct != null) {
      return `Фьючерсы под контролем: ${count}/3 позиций, вес ${weightPct}% (лимит 10%), плечо в пределах ≤2x альты / ≤3x BTC-золото.`;
    }

...
export function calculateCategoryAllocations(
  ...
  return CATEGORY_ORDER.map((category) => {
    ...
    return {
      name: category,
      value,
      share: totalValue ? round(value / totalValue, 4) : 0,
    };
  });
}
...
export function calculateRisk(positions: PositionCalculated[]): Risk {
  const portfolioValue = round(
    positions.reduce((sum, item) => sum + item.currentValue, 0),
  );

  const reserve =
    positions.find((item) => item.category === "Свободные деньги")?.currentValue ?? 0;

  const reserveShare = portfolioValue ? reserve / portfolioValue : 0;

  const cryptoValue = positions
    .filter((item) => item.category === "Крипта")
    .reduce((sum, item) => sum + item.currentValue, 0);

  const metalsValue = positions
    .filter((item) => item.category === "Металлы")
    .reduce((sum, item) => sum + item.currentValue, 0);

  const futuresValue = positions
    .filter((item) => item.category === "Фьючерсы")
    .reduce((sum, item) => sum + item.currentValue, 0);

  const stocksValue = positions
    .filter((item) => item.category === "Акции")
    .reduce((sum, item) => sum + item.currentValue, 0);

  const workBudget = reserve * 0.4575;

  const largestRiskAsset =
    positions
      .filter((item) => item.category !== "Свободные деньги")
      .sort((a, b) => b.currentValue - a.currentValue)[0] ?? null;

  const health =
    reserveShare >= 0.5 ? 0.88 :
    reserveShare >= 0.35 ? 0.74 :
    reserveShare >= 0.2 ? 0.59 :
    0.41;
  ...
  const signal =
    reserveShare >= 0.5
      ? "Резерв высокий. Можно добирать ядро и держать спекулятивный лимит."
      : reserveShare >= 0.35
      ? "Резерв нормальный. Добор только ступенчато."
      : "Резерв низкий. Новые входы только выборочно.";

  const summary =
    reserveShare >= 0.5
      ? "Портфель защитный. Есть манёвренность и запас по риску."
      : reserveShare >= 0.35
      ? "Портфель сбалансирован, но агрессию лучше не повышать."
      : "Портфель уже нагружен. Приоритет - защита и дисциплина.";
